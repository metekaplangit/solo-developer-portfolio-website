...
export const site = {
  ...
  /** Home hero headline. Restates PRODUCT.md's confirmed positioning line
   *  ("A one-person studio shipping quietly excellent Mac software") as a
   *  proposition rather than repeating the studio name the nav already shows.
   *  Kept here, not inline, so the wording has one owner. */
  headline: 'Quietly excellent software for the Mac.',
  /** Home hero lede. `description` below is the SEO <meta> string and reads
   *  like one; this is the same substance written for a person. */
  lede:
